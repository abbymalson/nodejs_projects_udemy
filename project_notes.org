...
**** ChatIO User Interface
     ...
     http://socket.io/docs
     we're going to use sockets
     open connection between client and server
     makes everything pretty much instant

     using node with express
     create a new folder called chatio
***** index.html
     #+name: index.html
     #+begin_src html
     <!doctype html>
...
***** command line
     ...
     #+begin_src sh
     npm install
     # installs socket.io and express
     #+end_src
***** server.js
     #+name: server.js
     #+begin_src js
...
**** User Functionality
     we are creating uesrs in this section, so we are editing some of our old files
     server.js we are addding the usernames array to old all of the usernames
     (updated the files in the other headings ...)
**** Deploying an App with Heroku
     going over how to deploy to heroku (from windows)
     use installer
...
***** command line
     ...
     follow getting started steps in heroku
***** command line
     #+name: command line
     #+begin_src sh
...
**** Cassandra Install & Setup
     ...
     going over project
     how to use cassandra with nodejs
     ...
     column type database - nosqql database

     http://cassandra.apache.org/download -apache stacks

     run "Cassandra cql shell" as administrator
     create database
     key spaces
     #+name: importing external files
     #+begin_src sh
     cqlsh -f <file name> -k <keyspace name>
     #+end_src
     #+name: executing cqlsh on command line
     #+begin_src sh
     cqlsh -e "command"
     cqlsh -e "SELECT * FROM mytable"
     #+end_src
...
**** FindaDoc User Interface
     create a new folder htmldev
     create a new folder css, images
...
***** index.html
     ...
     <!DOCTYPE html>
        ...
        <div id="wrap">
          <div id="header">
            <h1><a href="#">FindaDoc</a></h1>
            <h2>Search for local Doctors</h2>
          </div>

          <div id="menu">
            <ul>
              <li><a href="index.html">Home</a></li>
              <li><a href="doctors.html">Browse Doctors</a></li>
          ...
          <div id="content">
            <div id="left">
              <h2>Welcome to FindaDoc</h2>
              <p>We will help you find a doctor in your area</p>
              <br>
              <form>
                <select>
...
***** style.css     
     ...
     #+end_src
     adding css, adding details ... 
**** Express Setup
node + express + node handlebars - alternative for jade
website with express framework
routing + http server
npm install -g express
nodemailer
jade - template engine
new folder - express_website
app.js
package.json
***** command line
     #+name: command line
...
npm install
      #+end_src
there is a node_modules folder
...
**** Express Layout
adding routes and templating

routes/index.js
index.html
views/layouts/main.handlebars

views/partials/categories.handlebars (it needs to be partials, you can change that setting, but we aren't going over that)
paste what we have and save it
...
views/add-doctors.handlebars
views/add-categories.handlebars


now we are going to do cassandra step
**** Fetch Doctors From Cassandra
so now we want to grab doctors from databaseConfig

***** routes/doctors.js
#+name: routes/doctors.js
#+begin_src js
...
router.get('/details/:id', function(req, res, next) {
  ...
  client.execute(query, [req.params.id], function(err, results) {
    if(err) {
      res.status(404).send({msg: err});
    } else {
      res.render('details', {
...
{{#each doctors}}
  <li>
    ...
    <p><a href="/doctors/details/{{doc_id}}">&gt; details</a>
  </li>
{{/each}}
...
global query, so in app.js
#+name: global variables node
#+begin_src js

var app.local.cats - global variable
#+end_src
**** Add & Search Doctors
***** /routes/doctors.js
     #+name: /routes/doctors.js
...
make a password for root, because best practices
...
url varchar 255
date date
create_date datetime current_time

now we have a projects table
